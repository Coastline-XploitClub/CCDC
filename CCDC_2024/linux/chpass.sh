...
DRYRUN="false"
USEFILE="false"
USERS=$(cat /etc/passwd | grep sh$ | grep -v root | awk -F':' '{print $1}')  # Get all users (not "root") with shells from /etc/passwd (Default behavior)
...
while getopts "hsdf:" option; do
   case $option in
      ...
      d) # Dry run
         ...
         ;;
      s) # Use shadow
         USERS=$(cat /etc/shadow | grep '^[^:]*:[^\*!]' | grep -v root | awk -F':' '{print $1}')  # Get all users (not "root") with passwords from /etc/shadow
         ;; 
      f) # Use file
         USEFILE="true"
         FILE=$OPTARG
